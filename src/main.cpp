...
uint256 hashGenesisBlock("0x1bdfb24bfe94286cf4667c5a5de36e08c3da95cc3b82814790c623562c34fb85");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Makicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Makicoin Signed Message:\n";
...
{
    // Makicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2102400 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 2102400);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // 1 dag
static const int64 nTargetSpacing = 1 * 60; // Makicoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Makicoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    /// debug print
    printf("GetNextWorkRequired_V1 RETARGET\n");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Makicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Makicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "One box to go with some extra maki please";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 575122300; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 205924;

        if (fTestNet)
        {
            block.nTime    = 575122255;
            block.nNonce   = 119313;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc6e9e64e4f0c1e0a01f651189002e64276dfa67d5dd0ce51fb25c7cfd3e22e2b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Makicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("makicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("makicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static makicoinMiner(CWallet *pwallet)
{
    printf("makicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("makicoin-miner");
    ...
    try { loop {
        ...
        printf("Running makicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("makicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&makicoinMiner, pwallet));
}
